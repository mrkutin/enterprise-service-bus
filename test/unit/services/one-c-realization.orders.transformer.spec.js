...
const ServiceSchema = require('../../../services/one-c-realization.orders.transformer.service')

describe('one-c-realization.orders.transformer service (unit)', () => {
	let service
	...
	describe('method: aggregate', () => {
		it('returns recids as-is when table is one-c-act-provision-production-services', async () => {
			const recids = ['X1','X2']
			const out = await service.aggregate('one-c-act-provision-production-services', recids)
		...
		it('builds pipeline and flattens recids for dirpartytable', async () => {
			...
			service.getRecords.mockImplementationOnce(async (table, pipeline) => {
				...
				return [{
					'account-one-c-act-provision-production-services': [{ recid: 'A1' }],
					'consignee-one-c-act-provision-production-services': [{ recid: 'B1' }]
				}]
			})
		...
		it('builds pipeline and returns recids for extcode', async () => {
			...
			service.getRecords.mockImplementationOnce(async (t, p) => {
				capturedPipeline = p
				return [{ 'one-c-act-provision-production-services': [{ recid: 'E1' }, { recid: 'E2' }] }]
			})
	...
	describe('action: process', () => {
		it('sends transformed recids to channel when aggregate returns ids', async () => {
			...
			await ServiceSchema.actions.process.call(service, {
				action: { name: 'one-c-realization.orders.transformer.process' },
	...
	describe('channels', () => {
		...
		it('one-c-act-provision-production-services-topic puts recid into bucket', async () => {
			const handler = ServiceSchema.channels['one-c-act-provision-production-services-topic'].handler
			const callSpy = jest.spyOn(service.broker, 'call').mockResolvedValue()
			await handler.call(service, { channelName: 'one-c-act-provision-production-services-topic', params: { recid: 'O1' } })
			expect(callSpy).toHaveBeenCalledWith(
				`${service.name}.putKey`,
				{ bucket: `${service.name}:one-c-act-provision-production-services`, key: 'O1' }
			)
		})
