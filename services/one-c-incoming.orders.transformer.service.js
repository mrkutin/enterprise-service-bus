...
const {
	REDIS_ONE_C_INCOMING_ORDERS_RECIDS_TO_TRANSFORM_READ_COUNT
} = process.env

const dirpartytableAggregation = [
	{
		$lookup: {
			from: 'one-c-customers-order',
			localField: 'partynumber',
			foreignField: 'контрагент.IDAX',
			as: 'account-one-c-customers-order'
		}
	},
	{
		$lookup: {
			...
			as: 'consignee-one-c-customers-order'
		}
	},
	{
		$project: {
			'account-one-c-customers-order.recid': 1,
			'consignee-one-c-customers-order.recid': 1,
			'_id': 0
		}
	}
]
...
const extcodeAggregation = [
	{
		$lookup: {
			...
			as: 'one-c-customers-order'
		}
	},
	{
		$project: {
			'one-c-customers-order.recid': 1,
			'_id': 0
		}
	}
]
...
module.exports = {
	...
	settings: {
		recidsToTransformReadCount: REDIS_ONE_C_INCOMING_ORDERS_RECIDS_TO_TRANSFORM_READ_COUNT
	},
		...
		async process(ctx) {
			...
			if (aggregateResult.length) {
				for (const recid of aggregateResult) {
					await this.broker.sendToChannel('channel.one-c-incoming.orders.recid.transformed', { recid }, {key: `${recid}`})
				}
			} else {
	...
	channels: {
		...
		'one-c-customers-order-topic': {
			...
			handler(ctx) {
				...
				this.logger.info(`Channel ${ctx.channelName}: ${JSON.stringify(ctx.params)}`)
				this.broker.call(`${this.name}.putKey`, {
					bucket: `${this.name}:one-c-customers-order`,
					key: record.recid
				})
			}
		},
